...
This combined variant includes two dependency-free extensions:

  1. Graded Cartesian mesh (piecewise-uniform spacing via
     build_graded_coords) -- the intermediate step of README.md §11:
     finer resolution near conductor edges and corners while keeping
     the same structured-triangle assembly path.

  2. §10.4 boundary-classification hardening -- every Shape.contains()
     comparison uses a small absolute tolerance (_BOUNDARY_TOL) so that
     two arithmetically equivalent ways of computing the same boundary
     coordinate classify nodes identically. A small stress-test runs
     at startup to demonstrate the fix.

Full documentation -- the physics, math, numerical method, architecture,
usage examples, and validation results -- lives in README.md. This file's
comments are intentionally brief pointers; only the function/class
docstrings below are meant to be self-contained.
...
--------
     ...
     2. GEOMETRY         Shape (base, with CSG |, &, - operators),
                          ...
                          *** §10.4 tolerance applied in every contains() ***
     3. MATERIALS        Material, make_eps_r_function()
     4. MESH             snap_to_grid(), build_graded_coords(),
                          structured triangular Mesh (uniform or graded)
     ...
     9. EXAMPLES         parallel-plate (with graded production solve) +
                          coax + exact-solution check + §10.4 stress test

Dependencies: numpy, scipy, matplotlib.
Run directly: python3 capacitor_fem_graded_tol.py
"""
...
def build_graded_coords(segments):
    ...
    contiguous segments of (possibly different) uniform spacing.

    Parameters
    ----------
    segments : list of (start, end, n_points)
        Each segment is linspace(start, end, n_points). Consecutive
        segments must satisfy end_of_previous == start_of_next (within
        floating-point tolerance). The shared junction point is included
        only once.

    Returns
    -------
    ndarray
        Sorted unique coordinates covering the whole interval.

    This is the dependency-free "graded Cartesian" primitive: denser
    segments near conductors / corners, coarser segments far away.
    The resulting xs/ys are handed to Mesh, which otherwise stays
    unchanged. See README.md section 11 (graded structured mesh).
    """
    if not segments:
    ...
    for i, (a, b, n) in enumerate(segments):
        ...
        n = int(n)
        if n < 2:
            raise ValueError(f"build_graded_coords: segment {i} needs n >= 2, got {n}")
        ...
        if i > 0:
            part = part[1:]  # drop duplicate junction
        parts.append(part)
        prev_end = b
...
def _warn_if_large_mesh(n_nodes, n_tris):
    ...
    if n_nodes >= 5_000_000:
        print(f"WARNING: this mesh has {n_nodes:,} nodes ({n_tris:,} triangles). "
              ...
              f"sparse solve (scipy.sparse.linalg.spsolve) on a structured grid. "
              f"A general LU factorization's memory use grows faster than the node "
              f"count itself. If this is more than your machine can handle: use a "
              f"coarser mesh_spacing, or use the graded Cartesian option "
              f"(build_graded_coords + Mesh(xs=..., ys=...)) already implemented "
              f"in this file, or see LIMITATIONS AND FUTURE WORK / README.md "
              f"sections 10-11 for an unstructured mesh.")
...
class Mesh:
    """A structured triangular mesh spanning a rectangular domain,
    built from an (nx-by-ny) grid of nodes that may be *uniform* or
    *graded*.

    Uniform construction (original behaviour):
        Mesh(x0, y0, Lx, Ly, nx=nx, ny=ny)

    Graded construction (dependency-free intermediate step of README §11):
        xs = build_graded_coords([...segments...])
        ys = build_graded_coords([...segments...])
        Mesh(x0, y0, Lx, Ly, xs=xs, ys=ys)
        # or simply Mesh(xs=xs, ys=ys) -- the x0/y0/Lx/Ly are then ignored
        # for coordinate generation but still accepted for API compatibility.

    Each rectangular cell is split into two triangles, alternating the
    diagonal in a checkerboard pattern so the mesh has no built-in
    directional bias. All downstream FEM routines (assembly, fields,
    energy) are agnostic to whether the spacing is uniform or graded;
    they only consume self.points and self.triangles.
    """
...
def _solve_parallel_plate(config, h, use_graded=False):
    """Build and solve the parallel-plate problem in `config` at nominal
    grid spacing h.
    ...
    If use_graded=True, a piecewise-uniform (graded Cartesian) mesh is
    built instead of a globally uniform one: finer spacing near the plate
    edges (fringing / corner singularity) and through the gap, coarser
    spacing in the far-field margins. Total node count stays comparable
    to the uniform mesh at the same nominal h. This is the dependency-free
    intermediate step described in README.md §11.
    """
    ...
    if use_graded:
        # Graded Cartesian construction.
        # Design goals:
        #   - exact inclusion of all snapped geometry lines (so contains()
        #     classification stays exact for axis-aligned features)
        #   - roughly 2-3x finer local spacing near plate ends (corners)
        #     and through the gap / dielectric interface
        #   - coarser far-field margins
        #   - total nodes comparable to the uniform mesh at the same h
        #
        # x-direction: coarse margins, fine edge bands, medium plate interior
        edge_band = max(4 * h, 1.5e-3)          # width of refined zone at each plate end
        n_margin_x = max(4, round(margin / (2 * h)) + 1)
        n_edge = max(6, round(edge_band / (0.5 * h)) + 1)
        n_interior = max(8, round((plate_w - 2 * edge_band) / (1.2 * h)) + 1)
        # Ensure non-negative interior length
        if plate_w <= 2 * edge_band:
            edge_band = 0.25 * plate_w
            n_edge = max(6, round(edge_band / (0.5 * h)) + 1)
            n_interior = max(4, round((plate_w - 2 * edge_band) / h) + 1)

        xs = build_graded_coords([
            (0.0, x_plate0, n_margin_x),                                    # left margin
            (x_plate0, x_plate0 + edge_band, n_edge),                       # left edge band
            (x_plate0 + edge_band, x_plate0 + plate_w - edge_band, n_interior),  # plate interior
            (x_plate0 + plate_w - edge_band, x_plate0 + plate_w, n_edge),   # right edge band
            (x_plate0 + plate_w, Lx, n_margin_x),                           # right margin
        ])

        # y-direction: coarse outer margins, medium through plates,
        # fine through the gap (and dielectric interface)
        n_margin_y = max(4, round(margin / (2 * h)) + 1)
        n_plate = max(4, round(plate_t / (0.8 * h)) + 1)
        n_gap = max(10, round(gap / (0.45 * h)) + 1)   # finer in the gap

        ys = build_graded_coords([
            (0.0, margin, n_margin_y),                  # bottom margin
            (margin, y_gap_lo, n_plate),                # bottom plate
            (y_gap_lo, y_gap_hi, n_gap),                # gap (incl. dielectric)
            (y_gap_hi, y_gap_hi + plate_t, n_plate),    # top plate
            (y_gap_hi + plate_t, Ly, n_margin_y),       # top margin
        ])
    ...
    return dict(h=h, mesh=mesh, conductors=conductors, eps_r_of_xy=eps_r_of_xy,
                ...
                gap=gap, margin=margin, graded=use_graded)
...
def example_parallel_plate(config=None):
    ...
    C_uniform, C_ideal = result["C"], result["C_ideal"]
    print(f"Uniform mesh (finest clean level): {result['mesh'].n_nodes} nodes, "
          f"{result['mesh'].n_tris} triangles (h = {result['h'] * 1e3:.3f} mm)")
    ...
    # Graded Cartesian production solve at the same nominal finest h.
    # Finer local spacing near plate ends (corner singularity) and through
    # the gap; coarser far-field. Total node count stays comparable.
    print("Graded Cartesian mesh (same nominal h, refined near edges & gap):")
    graded = _solve_parallel_plate(config, result["h"], use_graded=True)
    C = graded["C"]
    print(f"mesh: {graded['mesh'].n_nodes} nodes, {graded['mesh'].n_tris} triangles "
          f"(graded, nominal h = {graded['h'] * 1e3:.3f} mm)")
    print(f"FEM capacitance (graded)  : {C * 1e12:9.3f} pF/m")
    print(f"Delta vs uniform          : {100 * (C - C_uniform) / C_uniform:+.2f} %")
    ...
    print("it improves resolution of the corner singularity without adding any "
          "native dependency.")
...
def verify_boundary_tolerance():
    """§10.4 stress test: deliberately perturb a boundary coordinate by a
    few ULPs and confirm that Shape.contains() still classifies the node
    the same way, thanks to _BOUNDARY_TOL.

    This is the exact failure mode described in README.md §10.4 (and the
    reason the ParallelPlateConfig auto-derivation of convergence_spacings
    was made deliberately conservative). Without the tolerance an entire
    row of nodes can flip classification; with it the classification is
    stable.
    """
    print("=" * 72)
    ...
    # A plate edge at a "nice" value that can be recomputed two ways.
    x0 = 0.015          # 15 mm
    width = 0.024       # 24 mm
    y0 = 0.016
    height = 0.001
    ...
    # Exact right-hand edge coordinate.
    x_edge = x0 + width          # 0.039

    # Reconstructions that are *mathematically* equal to x_edge but can
    # differ by a few ULPs in float64 (the classic §10.4 failure mode).
    # 1.5 * 0.1e-3 is the exact pattern that broke an earlier version of
    # ParallelPlateConfig's convergence_spacings auto-derivation.
    candidates = [
        ("literal 0.039",           0.039),
        ("1.5 * 0.026",             1.5 * 0.026),
        ("39e-3",                   39e-3),
    ...
    print(f"_BOUNDARY_TOL = {_BOUNDARY_TOL} m")
